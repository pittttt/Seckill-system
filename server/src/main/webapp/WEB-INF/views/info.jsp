...
<div class="container">
    ...
    <td>
        <c:choose>
            ...
            <c:otherwise>
                <a class="btn btn-info" style="font-size: 20px">商品已抢购完毕或者不在抢购时间段哦!</a>
            </c:otherwise>
        </c:choose>
    </td>
</div>
    ...
    function executeKill() {
        $.ajax({
            ...
            success: function(res){
                if (res.code==0) {
                    //alert(res.msg);
                    window.location.href="${ctx}/execute/success"
                }else{
                    alert(res.msg);
                    window.location.href="${ctx}/execute/fail"
                }
            },
